...
* 書いてもらった一発で動けば儲けもの。3回以内の書き直しで終われば効率的。それ以上はハマりゾーンなので自分でよく読んでデバッグ委s他方が良いです。
...
* これまでの文章と、学習済みの膨大な文書をもとに**次の1文字だけ**を考える機械。
  * これまでの文章の長さは、厳密な長さ制限があり、それを超えるとどうにもならない(現状196Kiトークンとか)
...
* 入力できる文章の長さは、学習する時に最初に決めて、それでクラウドに1億ドルとか払って計算をするので、あとで変えるわけにはいかない。
  * 増やすのが一番難しい値。
...
* 英語で支持するように作ったのに、日本語も覚えたら勝手に日本語の指示も聞いてくれる。
* 「段階的に考えて」と指示を出すだけで割と論理的な文章を書いてくれる。
* 計算もできる。繰り返しますが、できるように設計してません。
* 表が読める。
* コードが書ける。修正もできる。
* タスク分解ができる。
...
* これクラウドにお金をつぎ込むだけで、すべての面で人間を超えるんじゃないか？という夢を抱かせた。
...
* 繰り返しますが、今までのコンピューターのイメージは捨てろ。
  * 例えば計算は苦手。
  * 記憶力についてもよく忘れる。
  * 論理的な思考は全然できない。(ここで言う「論理」は、演繹的思考の意味)
...
* 文法
  ...
  * 日本語の単語も実は文法として覚えているらしい
    ...
    * 「人」＋「間」＝ 「人間」という法則を文法と同じように覚えている。
### 実はできない事
#### 計算
* 実は計算機能は全くつけてません。
* なのになぜかできるように。
* 人間と同じように、2+3と書くと5を連想する、みたいに考えているらしいです。
* 実は二桁くらいしか計算できない。人間と同じくらいですね。
  * 計算を間違えるのはAIとしてあまりに
    * ChatGPT5あたりから結構ややこしい計算でも間違えなくなりました。
    * ここでも別の関数呼び出しのような仕組みはないので、あくまで文法の一環として計算法則をトレーニングして覚えさせたらしいのです。
#### 論理思考
##### 論理的な話はできる場合も多い
...
* これができないので、プログラムは書けてもデバッグができない。
  ...
  * コンパイル、デバッグ環境を持たないのは、サンドボックス作るのに手間取っているのかと思いきや、やってもあまり効果がないので失望されないように作ってない面が大きいらしい。
#### 実行機能
...
* まあ人間でも、自信がないことは、嘘じゃない範囲で適当にごまかすことあるでしょ？
  ...
  * AIには、どこまでごまかていいかがわかっていません。
* 現状のAIのアルゴリズムには、自分が言っていることに自信があるかないかを把握する能力が含まれていない。
  * すべて自信を持っていうか、すべて自信投げに言うかどちらかしかなく、それなら自信ありげに言わないと売れない。
...
* 数百行プログラムなら雑に頼めば書いてくれます。
  ...
  * 論理的なデバッグはできないので、何度いっても直らなければその時には複雑すぎたなと思ってあきらめたほうがいいです。
