...
* 他のAIの話は別な人が調べてください。
  * 細かい使い勝手ではなく、本質的に何ができるかの話なので、そんなには変わらないはずと思っていますが。
* 書いてある知識のソースはだいたいChatGPTです。
  * 春市ネーションが混ざらないように気を付けはしました。
## 1. AIを便利に使う
### 最初に
...
### 結論から
* 長くなるのでまず結論
* あらゆる文章の中から、最もありそうでもっともらしい文章を出力します。
  * ただし、学者何万人分もの文章を学習した結果思う「もっともらしい」。
  * 平均的にかなり知的よりなので役に立ちます。
### LLMと普通のコンピューターの違い
...
* なんか今までのコンピューターの印象で、できると思われていることがたくさんありますが、以下のことはできるように設計されていません。
  ...
  * プログラムを見せるとの関数呼び出しとか全部網羅して解析して理解してくれそうな印象を持つ人がいますが、全くそんな機能はありません。
...
* 最近長く考えるようになって見えるのは、これを適宜繰り返したりしてます。
  * ChatGPTだとThinkingとかProとか選べるやつです
...
* 入力できる文章の長さは、学習する時に最初に決めて、それでクラウドに1億ドルとか払って計算をするので、作り直すときでないと変えるわけにはいかない。
  * 増やすのが一番難しい値。
* ChatGPT5の中でも頭がいい方で、196Kiトークンに決まっています。モデルを作り直さないと、それ以上は絶対に把握できない。
### 計算量
...
* 裾野が広ければ頂上が高くなる。
* 飛躍は10年に一度レベルで、たまたま出るのを待つしかありません。
  * それを取りこぼさないように研究テクニックはありますが。
### 創発（プログラマー向けのたとえ）
#### CPUの中には論理回路しかない
...
* 責めると、これから気を付けますとか言い出すけど信用してはいけない
  * 気を付けるというのがハルシネーション
  * これからは出典を調べて分かったことだけ記述し、出典を張りますとか言い出しますが
    * 出典見たら書いてないことが結構あります
    * そういう発言自体がハルシネーション
  * 丁寧に謝罪して対策を述べる文章の学習は大量にしています
    * 対策を実行したかどうかの学習は全くしていない
  * そういう人生を送ってきたんだから温かい目で
#### 対策
* まずAIにハルシネーションを聞いてみる
  * 割と正直に答えてくれます
    * その答えがハルシネーションではないという保証はないですが。
    * 前に話したときにどんな根拠が、とか思いだす機能はないはず。
* AIを誘導しないように気を付けて聞いてみる
  * 言っていることがいろんな聞き方をしても一貫していれば確度が高い
  * 自信がなくて言っていると言っていることがころころ変わる
* AIの学習データを想像する
  * AIの開発元が学習に使っているかどうか
  * 例えばAI関連の最新論文は全部社内に揃えているはず。
  * 多くのデータを学習した分野では正しいことを言います
* 基本的にはこれまでの文章の続きとして、もっともらしい文章かどうかで判別して出す文章を決めている事を忘れなければうまくいきます。
## 5.プログラミングをサポートしてもらう
### AIによるプログラミングの問題と解決法
- AIに任せきりにすると何が起きるか
  - コードがぐちゃぐちゃになる。
  ...
  - 破綻する。
  - 細部は違うとはいえ、原理は今まで人間が破綻してきたのと同じですね。
- 解決方法
  - プロセスとして管理する方法は今はTDD一択
  - これも今までと同じです。
...
* エラーメッセージや症状を言うと、直し方は教えてくれます
  * これで直れば問題なし
  * ただし難しいときは同じことを繰り返して堂々巡りになり、はまります
    * つまり人間と同じ
* 何より速いです。
  * 長くても5分しかかからないので、コストが低いのは間違いない
  * 何回か繰り返しても、元が取れることは多い
  * これで直れば問題なし
  * はまって永久に繰り返したらもちろんコストがかかります
    * 質問のコストもただではない
### 苦手な事：論理的なデバッグ
* よく考えて原因を絞り込んで少ない修正で直す、ということはできません
* LLMは、本質的に演繹思考ができません。
...
* りふぁくたりんぐを使ってできるパターンの解説とかやらせると、ものすごくうまいんですけどね
  * 書籍見て学習しているだけなので
### 苦手な事：作業プロセスの管理
...
* Red Green Refactoringのサイクルを意識して書いてもらう
  ...
  * 気を付けないといけないところも同じ
    ...
    * Greenでは余計なコードまで書かないように注意しておく必要がある
      * ここで動く最低限以上に付け足したコードはテストで機能が担保されていない
...
### 何が問題でできないのかを予測しながら進める
  - イエスマン。なんでもすぐわかりまずと言っていて技術的な回答としては使えない
    - 技術用の学習をやり直せば多分解決
    - 数個のモデルを同時に使うようにCEOが決断した時が解決の時。
  - ハルシネーションを繰り返す
    - 回答の温度の測定をしてそれに合わせて回答の生成のし直しをする必要が
    - うまくいく方法を数年かけてブラッシュアップして解決
  - 演繹思考が苦手
    - 現在のアルゴリズムに演繹を埋め込む余地がない
    - 新しいアルゴリズムを思いつく必要がある
### 原因から解決へのタイムスパンが予測できる
- 単に学習の予算が取れないだけ
  - 次のモデルで解決しても不思議はない
- アルゴリズムの使い方を工夫しないとけない
  - 数年単位で着実に解決
- transformerアルゴリズムそのものが悪い
  - 10年単位で解決はしないだろう
予測しながら学習を進めることが大事。
### 今後のプログラミングの学習
- AIの進歩に伴い、どんどん変わる最適解に合わせていく必要がある
  - ますますプロセスの理解が必要
